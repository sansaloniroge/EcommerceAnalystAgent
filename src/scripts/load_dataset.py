...
# (csv filename, table name, columns in schema.sql order). Order matters:
# parents before children, for both TRUNCATE ... CASCADE and the FK-safe
# insert order below.
...
def _rows_for_copy(csv_path: Path, columns: list[str]) -> list[tuple[Any, ...]]:
    ...
    with psycopg.connect(database_url) as conn:
        with conn.cursor() as cur:
            ...
            for csv_name, table, columns in TABLES:
                ...
                rows = _rows_for_copy(csv_path, columns)
